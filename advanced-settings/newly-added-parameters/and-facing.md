...
\
`&facing` takes priority over [`&videodevice`](../source-parameters/videodevice.md) , but it will fail to [`&videodevice`](../source-parameters/videodevice.md) or the default behaviour if the rear/front camera can't be selected automatically. If there are multiple rear or front cameras, it will use the first one. [`&videodevice=0`](../source-parameters/videodevice.md) will disable the video outright.
