...
This is not the same as [`&totalroombitrate`](totalroombitrate.md) / `&trb`. Use `&totalroombitrate` to raise or lower the total room viewing budget. Use `&roombitrate` on one guest's link to cap how much other guests can request from that specific guest.
