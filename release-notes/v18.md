...
*   `&tips` will show a help-screen on the guest joining (via @jcalado)

    &#x20;Also a toggle in the director's room is available to enable this.
...
* Added a somewhat experimental flag `&ltb` or `&limittotalbitrate` , which _tries_ to limit the total outbound bitrate to some max total value, via the publisher's side. This could be useful if you are broadcasting video as a director to the room, but only have a fixed amount of upload bandwidth or CPU. \
  ...
  \
  Example use: `?director=asdfdfssdfddd&trb=1000ltb=5000&`
...
* A socket server has been developed and provided that can be used as a personal handshake server for this use case. Documentation included. [https://github.com/steveseguin/websocket\_server](https://github.com/steveseguin/websocket\_server)
