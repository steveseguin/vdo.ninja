...
*   Created a new parameter called [`&totalscenebitrate`](../newly-added-parameters/and-maxtotalscenebitrate.md) , with aliases `&maxtotalscenebitrate`, `&tsb`, and `&mtsb`. Mainly added to help offer another way to optimize performance and limit inbound bandwidth used, since why not.
    ...
    If [`&videobitrate`](../advanced-settings/view-parameters/bitrate.md) is also used, `&videobitrate` becomes a max limit on any individual video, so you can set [`&tsb=6000`](../newly-added-parameters/and-maxtotalscenebitrate.md) and `&videobitrate=2000`, to keep all videos below 2-mbps each, but potentially go lower if more than 3 videos are present.&#x20;
...
* If you set the [`&maxvideobitrate`](../source-settings/maxbitrate.md) to less than 2500, on the sender side, it switches to a low bitrate and lower latency mode (500-ms chunk size). The default is otherwise 3500 with a 2-second chunk size. Obviously the quality will be worse.
