...
* `&miconly` will block the cycle-camera button and hotkey option now; also added a fail-safe to ensure the camera can't become active now.&#x20;
...
* increased the bitrate allotted to screen shares in a video mix, so the main screen share now uses up around 66% of the total bitrate cap, if there is a total bitrate cap. Before the bandwidth was divided equally to all the videos in the mix, but since screen share take up 66% of the screen now, I increased the bitrate to match.
...
* fixed an issue with `&meshcast` where a race-condition caused the video to not load for some viewers.&#x20;
...
* Added hotkey auto-response commands that work with twitch/youtube/facebook, which can be issued using a Streamdeck or DJ controller.
